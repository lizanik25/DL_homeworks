...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        encoded_text = self.indices[item]
        length = len(encoded_text) + 2
        encoded_text = [self.bos_id] + encoded_text
        encoded_text = encoded_text + [self.eos_id]

        if length > self.max_length:
            encoded_text = encoded_text[:self.max_length - 1] + [self.eos_id]
            padded_tensor = torch.tensor(encoded_text, dtype=torch.long)
            length = len(encoded_text)
        else:
            padded_tensor = torch.full((self.max_length,), self.pad_id, dtype=torch.long)
            padded_tensor[:len(encoded_text)] = torch.tensor(encoded_text, dtype=torch.long)
